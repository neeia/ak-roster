...
let domains: Partial<
  Record<ArknightsServer, Partial<Record<ArknightsDomain, string>>>
> = {};
...
async function request(
  ...
  if (url.includes("{0}")) {
    url = url.replace("{0}", "Android"); // Replace with actual platform value
  }
...
async function loadNetworkConfig(
  server: ArknightsServer | "all" | undefined = undefined
): Promise<void> {
  server = server || "all";

  if (server === "all") {
    await Promise.all(
      Object.keys(NETWORK_ROUTES).map((s) =>
        loadNetworkConfig(s as ArknightsServer)
      )
    );
    return;
  }
...
async function loadVersionConfig(
  server: ArknightsServer | "all" | null = null
): Promise<void> {
  server = server || "all";
  ...
  if (server === "all") {
    await Promise.all(
      Object.keys(NETWORK_ROUTES).map((s) =>
        loadVersionConfig(s as ArknightsServer)
      )
    );
    return;
  }
...
abstract class Client {
  ...
  async requestPassport(
    endpoint: string,
    body: any | undefined = undefined
  ): Promise<any> {
    let response = await fetch(
      `${YOSTAR_PASSPORT_DOMAINS[this.server]}/${endpoint}`,
  ...
  async authenticatedRequest(
    ...
    return await request("gs", endpoint, data, {
      secret: this.secret,
      uid: this.uid,
  ...
  async getU8Token(channelUid: string, accessToken: string): Promise<string> {
    let body = {
      appId: "1",
      platform: 1,
      channelId: "3",
  ...
  async requestYostarAuth(email: string): Promise<void> {
    await this.requestPassport("account/yostar_auth_request", {
      platform: "android",
      account: email,
      authlang: "en",
    });
  ...
  async submitYostarAuth(
    email: string,
    code: string
  ): Promise<[string, string]> {
    let data = await this.requestPassport("account/yostar_auth_submit", {
      account: email,
      code: code,
    });
