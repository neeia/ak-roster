...
const RecruitableOperatorCard = React.memo((props: Props) => {
  ...
  const width =
    getTextWidth(
      name,
      JSON.stringify(appTheme("#ffffff").typography.caption).replace(
        /[\{\}]+/g,
        ""
      )
    ) * WIDTH_TO_PX;
  ...
  let opName = t ? (
    <abbr title={operatorJson[op.op_id].name}>{nameComponent}</abbr>
  ) : (
    nameComponent
  );
  ...
  return (
    ...
    >
      <Box
        sx={{
          ...
          borderBottom: `3px solid ${
            rarityColors[operatorJson[op.op_id].rarity]
          }`,
          position: "relative",
        }}
      >
      ...
      <Box sx={{ gridArea: "2 / 1 / 2 / span 2" }}>{opName}</Box>
      {showPotentials &&
        (op.potential ? (
          <Box
            sx={{
