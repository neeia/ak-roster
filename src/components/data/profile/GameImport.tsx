...
const GameImport = (props: Props) => {
  ...
  async function processGameData(userData: UserData) {
    ...
    for (let i = 0; i < supportsData.length; i++) {
      ...
      if (supportModuleName) {
        supportModule[supportModuleName] =
          roster[charInstanceId]!.equip[supportModuleName]!.level;
      }
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
      You can import your account data if your account is linked to a Yostar
      account. Doing this WILL log you out from the game, if you are currently
      logged in.
      <TextField
        id="Mail"
        sx={{
