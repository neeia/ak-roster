...
const EventsTrackerDialog = React.memo((props: Props) => {
    ...
    const HELP_INFORMATION =
        <>
            <p>
                Event tracker purpose is to help with tracking free income from unreleased future events, e.g. shops, various rewards.
            </p>
            <ul>
            ...
            <ul>
                <li>Supports export and import from other Arknights community data sources (like tracking sheets). Data should be compiled into the presented import format.</li>
            </ul>
        </>;
    ...
    const handleIconClick = (eventName: string, id: string) => {

        if (Number(id) < 30000 || Number(id) > 32000 ||
            itemsJson[id as keyof typeof itemsJson].tier != 3)
            return;
    ...
    const renderEvent = useCallback((name: string, index: number) => {
                                ...
                                {(_eventData.farms ?? []).map((id) => [id, 0] as [string, number])
                                    ...
                                    .map(([id, quantity]) => (
                                        <ItemBase key={`${id}${quantity === 0 && "-farm"}`} itemId={id} size={itemBaseSize}>
                                            {quantity != 0 && <Typography {...numberCSS}>{formatNumber(quantity)}</Typography>}
                                        </ItemBase>
                                    ))}
                            </Stack>
                        </Stack>
        ...
        handleDeleteEvent, handleEventNameChange]
    )
